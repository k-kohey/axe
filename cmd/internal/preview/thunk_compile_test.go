...
// TestThunkCompilation verifies that thunks generated by codegen.GenerateCombinedThunk
...
func TestThunkCompilation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			thunkPath, err := codegen.GenerateCombinedThunk(files, compileTestModuleName, dirs.Thunk, "", targetPath)
			if err != nil {
				t.Fatal(err)
			}
...
func TestThunkCompilation_PathEscaping(t *testing.T) {
	...
	thunkPath, err := codegen.GenerateCombinedThunk(files, compileTestModuleName, dirs.Thunk, "", srcPath)
	if err != nil {
		t.Fatal(err)
	}
